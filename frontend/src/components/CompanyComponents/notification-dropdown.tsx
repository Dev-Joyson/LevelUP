...
export function CompanyNotificationDropdown() {
      ...
      const fetchCompanyId = async () => {
        try {
          ...
          if (response.data?.data?._id) {
            ...
            console.log('Company ID fetched:', response.data.data._id)
          }
        } catch (error) {
  ...
  const fetchNotifications = async () => {
    if (!token || user?.role !== 'company' || !companyId) return
    
    try {
      setIsLoading(true)
      const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:5000'
      const response = await axios.get(`${API_BASE_URL}/api/notifications/company?limit=10`, {
        headers: { Authorization: `Bearer ${token}` }
      })
  ...
  const markAllAsRead = async () => {
    ...
    try {
      const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:5000'
      await axios.put(`${API_BASE_URL}/api/notifications/read-all/company`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      })
  ...
  const handleNotificationClick = (notification: Notification) => {
    ...
    if (notification.type === 'application_submitted' && notification.entityId) {
      router.push(`/company/applicants?applicationId=${notification.entityId}`)
    }
  ...
  useEffect(() => {
    if (token && user?.role === 'company' && companyId) {
      // Initialize socket connection
      socketService.connect(token)
        .then(() => {
          console.log('Socket connected for company notifications')
