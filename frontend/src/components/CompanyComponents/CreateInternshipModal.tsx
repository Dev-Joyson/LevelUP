...
const CreateInternshipModal: React.FC<CreateInternshipModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const addTagItem = (field: keyof typeof currentInputs) => {
  ...
  const removeTagItem = (field: keyof typeof formData, index: number) => {
    setFormData((prev) => ({
      ...prev,
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Updated Salary Inputs - No Spinners */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  handleInputChange("salary", { ...formData.salary, min: Number(value) || 0 })
                }}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => {
                  const value = e.target.value.replace(/[^0-9]/g, "")
                  handleInputChange("salary", { ...formData.salary, max: Number(value) || 0 })
                }}
          ...
          {/* Additional Settings - Removed Positions Field */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => {
                  const value = e.target.value.replace(/[^0-9.]/g, "")
                  handleInputChange("minimumGPA", Number(value) || 0)
                }}
                placeholder="e.g., 3.0"
              />
            </div>
          ...
          {/* Tag-Style Dynamic Arrays */}
          {(["requirements", "benefits", "preferredSkills"] as const).map((field) => (
            <div key={field}>
              ...
              <div className="space-y-2 mt-2">
                <div className="flex gap-2">
                  <Input
                    ...
                    onChange={(e) => setCurrentInputs((prev) => ({ ...prev, [field]: e.target.value }))}
                    onKeyPress={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault()
                        addTagItem(field)
                    ...
                    placeholder={`Type ${field.slice(0, -1)} and press Enter`}
                  />
                  <Button type="button" variant="outline" size="sm" onClick={() => addTagItem(field)}>
                  ...
                  {(formData[field] as string[]).map((item: string, index: number) => (
                    <Badge key={index} variant="secondary" className="flex items-center gap-1">
                      ...
                      <Button
                        ...
                        onClick={() => removeTagItem(field, index)}
                      >
                        <X className="h-3 w-3" />
                      </Button>
                    </Badge>
